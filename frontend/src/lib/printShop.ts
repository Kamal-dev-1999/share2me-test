...
/**
 * Sets the vendor's role in the database.
 * token is the NextAuth session JWT.
 */
export async function setRole(role: UserRole, _account?: string | null, token?: string): Promise<void> {
  if (!token) return;
  const vendorApiBase = process.env.NEXT_PUBLIC_API_BASE?.replace('/printshop', '') || 'http://localhost:3000/g2p/vendor';
  await fetch(`${vendorApiBase}/role`, {
    method: 'POST',
...
/**
 * Returns the vendor's role from /me response.
 * token is the NextAuth session JWT.
 */
export async function getRole(_account?: string | null, token?: string): Promise<UserRole | null> {
...
export async function getPrintJobs(
  ...
  const data = await apiGet<{ jobs: PrintJob[] }>(`/jobs${query}`, token);
