...
// `stops` = gradient face; `deep` = the darker "side face" used as a hard
// extrusion shadow — that offset is what makes the icons read as 3D, matching
// the beveled look of the brand logo.
const ITEMS: { href: string; icon: LucideIcon; label: string; grad: string; stops: [string, string]; deep: string }[] = [
  ...
  { href: "/p2p",          icon: Network,     label: "P2P",     grad: "grad-p2p",    stops: ["#fcd34d", "#f59e0b"], deep: "#b45309" },
  { href: "/g2p",          icon: Inbox,       label: "Portal",  grad: "grad-portal", stops: ["#4ade80", "#059669"], deep: "#047857" },
  { href: "/tools",        icon: Wrench,      label: "Tools",   grad: "grad-tools",  stops: ["#e879f9", "#c026d3"], deep: "#86198f" },
  { href: "/how-it-works", icon: Compass,     label: "How",     grad: "grad-how",    stops: ["#38bdf8", "#0284c7"], deep: "#075985" },
  ...
  { href: "/pricing",      icon: Rocket,      label: "Pricing", grad: "grad-price",  stops: ["#a78bfa", "#7c3aed"], deep: "#5b21b6" },
];
    ...
    <>
      {ITEMS.map(({ href, icon: Icon, label, grad, deep }) => {
        ...
        return (
          ...
          >
            {/* Active state backplate — frosted light pill on the black dock */}
            {active && (
              <motion.span
                layoutId={layoutId}
...
export function SideRail({ embedded = false }: { embedded?: boolean }) {
  ...
  const defs = (
    <svg width="0" height="0" className="absolute pointer-events-none" aria-hidden="true">
      <defs>
        {/* Shared black 3D gradient — lit graphite top fading to near-black */}
        <linearGradient id="grad-ink" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop stopColor="#4B5563" offset="0%" />
          <stop stopColor="#0B0B0E" offset="100%" />
        </linearGradient>
        {ITEMS.map((item) => (
